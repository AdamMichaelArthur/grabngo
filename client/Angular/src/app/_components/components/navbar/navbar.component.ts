...
// export const ROUTEMENU: RouteInfo[] = [
//   {
//     path: '/user-profile',
//     title: 'Account Info',
//     icon: 'account',
...
export class NavbarComponent implements OnInit {
  ...
  getTitle() {
    ...
    return 'Sass product Main Dashboard';
  }

  //get navbar menu
  getAccount() {
    //this.menuItems = ROUTEMENU.filter(menuItem => menuItem);
    this.navService.getAccount().subscribe(
      (data: any) => {
        this.menuItems = data.account.navigation;
        console.log(data);
      },
      (error) => {
  ...
  linkhubspot() {
    ...
    this.navService.linkHubpost(userEmail).subscribe((data: any) => {
      this.menuItems = data.account.navigation;
      console.log(data);
    });
  ...
  getAccountSettings() {
    this.navService.getAccountSettings().subscribe(
      (data: any) => {
        if (data.Error === 505) {
